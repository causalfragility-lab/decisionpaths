...
#'   \item{path_frequencies}{Tibble of path strings, counts, and proportions.}
#'   \item{n_unique_paths}{Number of unique decision paths observed.}
#'   \item{by_group}{By-group entropy tibble (NULL if no group variable).}
...
dp_entropy <- function(x, by = NULL, mutual_info = FALSE) {
  ...
  .shannon <- function(strings) {
    strings <- strings[!is.na(strings) & nchar(strings) > 0L]
    if (length(strings) == 0L) return(0.0)
    freq  <- table(strings)
  ...
  # ── Overall entropy ───────────────────────────────────────────────────────
  H        <- as.numeric(.shannon(path_strings))
  n_unique <- as.integer(length(unique(
    path_strings[!is.na(path_strings) & nchar(path_strings) > 0L]
  )))

  # Always a plain numeric double — never integer, never NA silently
  H_norm <- if (n_unique <= 1L) 0.0 else H / log2(as.numeric(n_unique))
  ...
  if (!is.null(grp_var) && grp_var %in% names(x$paths)) {
    ...
    by_group <- do.call(rbind, lapply(grp_vals, function(g) {
      sub <- path_df[!is.na(path_df[[grp_var]]) &
                       path_df[[grp_var]] == g, ]
      H_g <- as.numeric(.shannon(sub$path_str))
      ...
      data.frame(
        ...
        entropy            = H_g,
        n_unique_paths     = n_g,
        normalized_entropy = if (n_g <= 1L) 0.0
        else H_g / log2(as.numeric(n_g)),
        stringsAsFactors   = FALSE
      )
    }))
...
print.dp_entropy <- function(x, ...) {
  ...
  cli::cli_text("Shannon entropy H   : {round(x$entropy,            3)} bits")
  cli::cli_text("Normalized entropy  : {round(x$normalized_entropy, 3)}")
  cli::cli_text("Unique paths        : {x$n_unique_paths}")
  if (!is.null(x$mutual_info)) {
    cli::cli_text(
      "Mutual information  : {round(x$mutual_info, 3)} bits"
    )
  }
