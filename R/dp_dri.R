...
#'
#' Computes the Decision Reliability Index (DRI), defined as one minus the
#' mean switching rate across units. A DRI of 1 indicates perfectly consistent
#' decisions; 0 indicates maximum instability.
#'
...
dp_dri <- function(x, by = NULL) {
  ...
  if (!is.null(grp_var) && grp_var %in% names(unit_tbl)) {
    ...
    by_group <- do.call(rbind, lapply(grp_vals, function(g) {
      sub <- unit_tbl[!is.na(unit_tbl$group) & unit_tbl$group == g, ]
  ...
  # ── Return — names(out) must include "group", "mean_switching_rate", "DRI" ─
  structure(
    list(
      group               = grp_label,
...
print.dp_dri <- function(x, ...) {
  ...
  cli::cli_text("Mean switching rate : {round(x$mean_switching_rate, 3)}")
  cli::cli_text("DRI                 : {round(x$DRI, 3)}")
